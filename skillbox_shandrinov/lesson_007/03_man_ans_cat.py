...
class Cat:
    ...
    def sleep(self):
        self.fullness -= 10
        cprint('{} спал'.format(self.name), color='blue')
    ...
    def eat(self):
        if self.house.cat_food >= 20:
            ...
            cprint('Нет кошачей еды', color='red')
...
class Man:
    ...
    def __str__(self):
        if self.cat == 0:
            ...
            return 'Я - {}, сытость {}.\nЯ - {}, сытость {}.'.format(
                self.name, self.fullness, self.cat.name, self.cat.fullness)

    def take_cat(self, name):
        cat = Cat(name)
        self.cat = cat
        self.cat.house = self.house
        self.cat.fullness -= 10
        self.house.cats += 1
        cprint('Кот {} вьехал в дом. Хозяин {}. В доме котов: {}'.format(self.cat.name, self.name, self.house.cats), color='cyan')
    ...
    def clean_house(self):
        self.house.dirt -= 100
        self.fullness -= 20
        cprint('{} убрал в доме'.format(self.name), color='blue')
    ...
    def act(self):
        if self.fullness <= 0:
            ...
            return
        dice = randint(1, 6)
        if self.house.cats == 0:
            self.take_cat('Мурзик')
        if self.house.cat_food < self.house.cats * 20:
            self.buy_cat_food()
        if self.fullness < 40:
            self.eat()
        if self.house.food < 10:
